...
SELECT distinct listing2.name, host.name
FROM listing2
INNER JOIN host ON listing2.host_id = host.id
where is_superhost = true;


SELECT name, count(listings_count)
FROM host
GROUP BY name
HAVING count(listings_count)>10;


select   sl.availability_365,l.availability_365
from  summary_listing2 sl ,listing2 l
where  sl.availability_365=l.availability_365 and sl.availability_365>1
order by sl.availability_365;
...
from host
full outer join listing2
on  listing2.id=host.id
order by host.id desc
limit 1000;


select geo.properties_neighbourhood ,nei.neighbourhood
from geolocation geo,  neighbourhood2 nei
where geo.properties_neighbourhood = nei.neighbourhood 
order by(geo.properties_neighbourhood);


SELECT distinct amenity.amenity_name, amenity.id
FROM amenity
FULL OUTER JOIN rm_connect
ON rm_connect.amenity_id = amenity.id
order by amenity.id ASC;

select p.listing_id,p.price
from price p,  listing2 l
